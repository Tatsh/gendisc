...
If any changed files are under `gendisc/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `gendisc/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `gendisc/commands/main.py`,
...
For Python files, strip the `gendisc/` prefix and replace `/` with `.` (like module imports).

- Python file `gendisc/media.py` → `media:`.
- Multiple files under `gendisc/commands/` → `commands:`.
- Single command file `gendisc/commands/admin.py` → `commands.admin:`.
