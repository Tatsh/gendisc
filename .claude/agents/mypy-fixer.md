...
Fixes mypy type errors and eliminates `Any` usage in the gendisc project.
...
1. Fix all mypy errors reported by `uv run mypy gendisc`
...
Place new TypedDict classes in `gendisc/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`gendisc/typing.py` when shared.
...
1. Run `uv run mypy gendisc` and capture all errors.
...
4. Run `uv run mypy gendisc` again to verify fixes.
