---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the gendisc project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the gendisc project.
...
- `` :py:mod:`gendisc.string` `` for modules
- `` :py:func:`gendisc.string.slugify` `` for functions
- `` :py:class:`gendisc.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~gendisc.string.slugify` `` renders as `slugify`
